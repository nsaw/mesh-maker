...
PRESETS = {
    ...
    # relief-starburst — mirrors the web preset's NON-radial settings so generated
    # Grasshopper definitions don't silently fall back to 'gentle-waves' (simplex) via
    # PRESETS.get(...)'s default branch when wired with the new preset name. The IronPython
    # sampler in noise_gen.py still doesn't read radial_* fields (radial-foci port deferred —
    # see PR #16 plan); this entry produces a plain Voronoi relief at the starburst base
    # settings, which is the correct degradation while the Python port is pending. Kept in
    # sync with grasshopper/components/presets.py.
