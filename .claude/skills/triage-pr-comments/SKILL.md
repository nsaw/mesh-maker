...
3. **Parse embedded comments from `<details>` dropdowns in comment bodies**:
   ...
   **The section vocabulary CHANGES — never hardcode it.** CodeRabbit moved to
   ...
   invisible. Sum every bucket's per-file entries; that total is what your findings
   list and coverage table must match.
...
gh api "repos/$OWNER_REPO/pulls/$PR/comments?per_page=100" --paginate \
  --jq '.[] | select(.body | contains("pr-review:v1:finding")) | {id, path, line, body}'

# Summary / review body (SOURCE 3 surface)
gh api "repos/$OWNER_REPO/pulls/$PR/reviews?per_page=100" --paginate \
  --jq '.[] | select(.body | contains("pr-review:v1:summary")) | {id, body, submitted_at}'
```
...
A round is not clean until the marker queries above return zero unresolved findings. Report the
`/pr-review` counts alongside the CodeRabbit and Greptile counts in the triage summary, and if a
marker query returns nothing, confirm `/pr-review` actually ran on this PR rather than recording
it as a clean pass.
